...
The one thing I want you to take seriously: **always point this at a copy of your footage, never your originals.** The tool is designed not to delete your files. The organizer only moves footage, and only after you approve a plan. But "designed not to" is not a guarantee, and I want us to be super careful with camera originals. You never know. So the rule is simple: I would never run this on footage that exists in only one copy, and neither should you.
...
- **Premiere Pro:** right-click your clips in the Project panel, then Proxy, then Create Proxies
- **Final Cut Pro:** File, then Transcode Media, then Create Proxy Media
- **DaVinci Resolve:** right-click in the Media Pool, then Generate Optimized Media

Or the simplest version: just duplicate your footage folder in Finder before you start. That gives you a real second copy, and that's what matters. Point the organizer at the copy. Keep your originals where they are. Once you're happy with how it runs, apply the same workflow to the real drives.
...
|-------|--------------|--------------|
| 🗂 **aod-footage-organizer** | Audits your drive against a professional folder structure, then, with your approval, moves footage into place. Every move is verified with a digital fingerprint, moves on the same drive are undoable with one command, and the skill is designed never to delete anything. | *"Check my footage folder"* · *"Organize this drive"* |
...
The three work as one bundle. **Organizer and analyst create the
...
These skills work with camera originals, which may be the only copy of irreplaceable
material. They are built around one rule: **they are designed never to delete your
files.** The organizer only moves footage, only after you approve a plan, and only
after you confirm a backup exists. Before and after every move it checks each file's
digital fingerprint, the same kind of verification professional offload tools use, to
prove nothing changed in transit. If you ever change your mind, just say **"undo the
organizer's moves"** and Claude reverses every move it made on that drive. Moves
*between* drives are even more conservative: the organizer copies and verifies, and
your original is never touched, so there is nothing to undo.

That said, "designed not to delete" is a design goal, not a promise of perfection.
Claude is running real commands on your real drive, and we want to be super careful.
So: **always keep a backup of your footage before organizing. Never run this on the
...
Cowork Skill Pack. Please act as my personal install assistant and walk me
through it ONE STEP AT A TIME. Wait for me to confirm each step before moving
to the next. I am not technical, so keep instructions simple and tell me
exactly what to click. Here is what we need to do together:

1. First, help me check that "Code execution and file creation" is turned ON.
   Tell me how to find it: Settings, then Capabilities, in this app. Wait
   until I confirm it's on.
...
footage folders. It is read-only until I approve moves, it is designed never
to delete files, and I should always work on a backup copy of my footage.
Remind me of the backup rule before step 5. Start with step 1 now.
```
...
2. Click the **+** button or the folder icon in the chat and give Claude access to your footage folder or drive. This is how Claude sees your files. You are granting access to a folder on your computer, and nothing gets uploaded.
3. Type exactly this:
...
Once you have the organizer running, install the other two skills the same way (Customize, then Skills, then +, then Upload a skill):
...
The status surface stays plain: folder tree, checkmarks, counts, and a short
sidebar summary like **"This folder is indexed: 3 files, 1 transcript, 2 people/tags,
all moves verified."** The reasoning stays in Claude; the visible UI only shows
...
These are the prompts to start with:
...
3. **"Transcribe these interviews"** → transcript with timecodes (first run installs
   the free transcription engine, and Claude walks you through it)
...
**[Wideframe](https://try.wideframe.com)** is an AI coworker for video editors. Where the AOD pack handles your raw footage (organizing, indexing, transcription), Wideframe goes further into the editorial layer: it understands your project semantically, lets you search your library by meaning, and can rough-assemble sequences from a natural-language description. It outputs real Premiere Pro project files with assembled timelines built from your own footage. I've been running it alongside this pack and they complement each other well: use the pack to get your footage organized and indexed, then bring Wideframe in when you're ready to start cutting. It's a paid app (Mac only, Apple Silicon), but if you're spending serious time in post, it's worth looking at.
